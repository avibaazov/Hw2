...
public class SensorActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSensorListener = new SensorEventListener() {
            ...
            public void onSensorChanged(SensorEvent event) {
                // Get the current values of the accelerometer sensor
                float x = event.values[0];
                float y = event.values[1];
                float z = event.values[2];

                // Calculate the inclination of the phone
                float inclination = (float) Math.sqrt(x * x + y * y + z * z);

                // If the inclination of the phone changes by a significant amount, move the car in the appropriate direction
                if (Math.abs(inclination - mLastInclination) > INCLINATION_THRESHOLD) {
                    if (x > 0) {
                        // Phone is tilted more towards vertical, move car to the right
                        moveCar(1);
                    } else {
                        // Phone is tilted more towards horizontal, move car to the left
                        moveCar(-1);
                    }
                }

                // Save the current inclination for the next time this method is called
                mLastInclination = inclination;
            }
    ...
    // In the onPause() method of your activity, unregister the SensorEventListener to stop listening for sensor changes
    @Override
    ...
    float mLastInclination;
    final float INCLINATION_THRESHOLD = 1.5f;
